...
def start_main({num_nodes,req}) do
 # nodes_number = num_node
  ...
  nodes_pid_map = Enum.map(1..num_nodes, fn n -> { "pid_#{n}",TapestryDos.DynamicNodeSupervisor.start_child_nodes([n,routing_map["rt_#{n}"]])} end)
  ...
  add_nodes_pid_map = Enum.map(num_nodes+1..num_nodes + add_nodes, fn n -> { "pid_#{n}",TapestryDos.DynamicNodeSupervisor.start_child_nodes([n,routing_map["rt_#{n}"]])} end)
